...
TestStomp::TestStomp(Boss* owner)
...
void TestStomp::Update()
{
	float deltaTime = Time::GetInstance().GetDeltaTime();

	switch (m_List)
	{
	case TestStomp::enAttack::None:
		//次の遷移への移動.
		m_List = enAttack::Stomp;
		break;
	case TestStomp::enAttack::Stomp:
		//ここで踏みつけ攻撃の動作を実装.
		BossAttack();
		...
		break;
	case TestStomp::enAttack::CoolTime:
		//クールタイム.
		//この時にdeltaTimeの設定をする.
		//deltaTimeの加算.
		m_Timer += deltaTime;
